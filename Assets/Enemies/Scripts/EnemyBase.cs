...
public abstract class EnemyBase : MonoBehaviour, IDamageable
{
    ...
    [Tooltip("Time (sec) from attack start to first hit.")]
    [SerializeField] private float attackHitTimeA = 0.25f;
    [Tooltip("Second hit time (sec). Set <0 to disable second hit.")]
    [SerializeField] private float attackHitTimeB = -1f;
    [Tooltip("Failsafe: we force-unlock attacking after this many seconds in case the end event is missed.")]
    ...
    // --- helpers used by states/trigger scripts ---
    ...
    // --- attack flow (animation or simulated) ---
    ...
    // Animation Events (only if you wire them on Attack.anim and set useAnimationEvents = true)
